...
function createPixelCells() {
  ...
  const targetCells = width < 760 ? 420 : 850;
  const pixelSize = Math.max(10, Math.ceil(Math.sqrt((width * height) / targetCells)));
...
function createArchiveTransition() {
  ...
  if (!archive || !archiveContent || !workCards.length || !intro || !nextInner || !pixelLayer || !pixelCells.length) {
    return;
  }
