  ...
  class PixelBreak {
    constructor(options = {}) {
      this.options = {
        ...
        timelineEnd: options.timelineEnd || "+=760%",
        debug: !!options.debug
      };
    ...
    step(now) {
      ...
      const done =
        (this.target === 1 && this.progress >= 0.999) ||
        (this.target === 0 && this.progress <= 0.001);
    ...
    draw() {
      ...
      for (const px of this.particles) {
        ...
        const x =
          px.ox +
          px.side * 315 * le * px.speed +
          px.dirX * 38 * le +
          Math.sin(px.oy * 0.038 + p * 8) * 72 * eased;

        const y =
          px.oy +
          Math.cos(px.ox * 0.026 + p * 7) * 24 * eased +
          Math.sin(px.ox * 0.03) * 30 * le -
          48 * le;
